...
export const AKADEMICALSPRAVKA_WIZARD_SCENE_ID = 'akademicalspravka_wizard_scene';
...
export const academicalSpravkaWizardScene = new Scenes.WizardScene<any>(
    AKADEMICALSPRAVKA_WIZARD_SCENE_ID,
...
academicalSpravkaWizardScene.action("back", async (ctx) => {
    ctx.answerCbQuery();
    await ctx.scene.enter(NEVIGATION_SCENE_ID).catch(TgBot.catchPollingError)
})

function buildFormUrl(params: IEduSpravka) {
    return `https://docs.google.com/forms/u/0/d/e/1FAIpQLSfEoltNW_5wqW7SZxmHwXW8RURS8aKAB47pNb1ezQuG8cquyA/formResponse?entry.1513514743=Да&entry.1264818593=${params.birth}&entry.824577011=${params.mail}&entry.638547864=${params.name}&entry.1445740066=${params.group}&entry.654044256=${params.objective}&entry.565348981=Кучин переулок, дом 14, аудитория 1206 (Сектор документообеспечения)`
}